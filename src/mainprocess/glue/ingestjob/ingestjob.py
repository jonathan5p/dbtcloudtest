...
def unique_by_merge_key(input_df: DataFrame, merge_key: str, precombine_keys: list):
    unique_per_id = input_df.orderBy(
        [merge_key, *precombine_keys], ascending=False
...
def full_load(
    ...
    id_df = (
        input_df.withColumn("dlid", F.expr("uuid()"))
...
def incremental_load(
    ...
    changes_df = spark.sql(
        f"select changes_df.*, target_df.dlid, target_df.dlingestionts, target_df.dllastmodificationts from changes_df left join target_df on changes_df.{merge_key}=target_df.{merge_key}"
    )
    ...
    changes_with_gid = (
        changes_df.withColumn(
            "dlingestionts",
            F.when(F.col("dlid").isNull(), current_ts).otherwise(
                F.col("dlingestionts")
            ),
        )
        .withColumn(
            "dlid",
            F.when(F.col("dlid").isNull(), F.expr("uuid()")).otherwise(F.col("dlid")),
        )
        .withColumn("dllastmodificationts", current_ts)
    )

    target_df.alias("target").merge(
        changes_with_gid.alias("source"), f"target.{merge_key} = source.{merge_key}"
...
if __name__ == "__main__":
    ...
    job = Job(glueContext)
    job.init(
        f"{args['JOB_NAME']}-{args['catalog_database']}-{args['catalog_table']}", args
    )

...
# # Download config files for agents
# s3.download_file(
#     args["config_bucket"],
#     f'agent/version_{args["model_version"]}/splink_model.json',
#     f"/tmp/splink_model.json",
# )
# s3.download_file(
#     args["config_bucket"],
#     f'agent/version_{args["model_version"]}/splink_config.json',
#     f"/tmp/splink_config.json",
# )

# splink_config = json.load(open("/tmp/splink_config.json"))
# print("Agent splink config: ", splink_config)

# # Get splink predict results for agents
# linker = SparkLinker(clean_df, spark=spark, break_lineage_method="parquet")
# linker.load_model(f"/tmp/splink_model.json")
...
# # Write data to S3
# individuals_df.write.\
#               mode("overwrite").\
#               format("parquet").\
#               option("path",f"s3://{args['data_bucket']}/consume_data/{args['glue_db']}/individuals/").\
#               option("overwriteSchema", "true").\
#               option("compression", "snappy").\
#               saveAsTable(f"{args['glue_db']}.individuals")

# Write data to the Aurora PostgreSQL database
username = ssm.get_parameter(Name=f"/secure/{args['ssm_params_base']}/username",WithDecryption=True)['Parameter']['Value']
jdbcurl = ssm.get_parameter(Name=f"/parameter/{args['ssm_params_base']}/jdbc_url")['Parameter']['Value']
password = ssm.get_parameter(Name=f"/secure/{args['ssm_params_base']}/password",WithDecryption=True)['Parameter']['Value']

...
def deduplicate_entity(entity: str, spark_df: DataFrame):
    s3.download_file(
        args["config_bucket"],
    ...
    linker.load_model(f"/tmp/{entity}_splink_model.json")
...
dedup_office_df = deduplicate_entity(entity="office", spark_df=office_df)
dedup_office_df.createOrReplaceTempView("dedup_office_df")

dedup_team_df = deduplicate_entity(entity="team", spark_df=team_df)
...
org_df.write.\
      mode("overwrite").\
      format("parquet").\
      option("path", f"s3://{args['data_bucket']}/consume_data/{args['glue_db']}/organizations/").\
      option("overwriteSchema", "true").\
      option("compression", "snappy").\
      saveAsTable(f"{args['glue_db']}.organizations")

...
def get_geo_info(row, geocode):
    ...
    if row.officecity is not None:
        query = row.officecity+", "+state_abb2name.get(row.officestateorprovince, "")
        location = geocode(query, country_codes = [row.officecountry], addressdetails=True, featuretype="")
...
def lambda_handler(event, context):
    ...
    delta_table = delta_table.set_index("ingest_unique_id")
    ...
    # Get valid CAAR data
    filter_df = delta_table.loc[((delta_table["officesubsystemlocale"]=="BRIGHT_CAAR") & (delta_table["officecity"]!="OTHER")),:].copy()
    ...
    filter_df.loc[:,"geo_info"] = filter_df.apply(get_geo_info, geocode=geocode,axis=1)
    filter_df.loc[:,"county"] = filter_df["geo_info"].apply(lambda x: x.get("county", x.get("city", x.get("town"))) if x != None else None)
    ...
    delta_table.loc[:,"uniqueorgid"] = delta_table.apply(lambda x: uoi_mapper.get(x.officesubsystemlocale,uoi_mapper["Other"]), axis=1)
